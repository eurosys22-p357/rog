...
def main():
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            logger = Logger(os.path.join(args.checkpoint, args.arch + '_log.txt'), title=title, resume=True)
        else:
            print("=> no checkpoint found at '{}'".format(args.resume))
    else:
        logger = Logger(os.path.join(args.checkpoint, args.arch + '_log.txt'), title=title)
        logger.set_names(
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train_loss, train_top1_acc, train_top5_acc, time_elapsed = train(train_loader, train_loader_len, model,
                                                                         criterion, optimizer, epoch)

        # evaluate on validation set
        val_loss, val_top1_acc, val_top5_acc = validate(val_loader, val_loader_len, model, criterion)
        ...
        is_best = val_top1_acc > best_prec1
        best_prec1 = max(val_top1_acc, best_prec1)
        save_checkpoint({
            'epoch': epoch + 1,
            'arch': args.arch,
